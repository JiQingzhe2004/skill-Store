...
export function SkillActions({
  slug, initialStarCount, initialLikeCount, initialDownloadCount,
  initialStarred, initialLiked, isLoggedIn,
}: Props) {
  ...
  const handleInstall = async () => {
    ...
    try {
      ...
      setInstallMsg(m.skillActions.installSuccess)
      setTimeout(() => setInstallMsg(''), 3000)
    } catch {
      setErrMsg(m.skillActions.installError)
      setTimeout(() => setErrMsg(''), 3000)
    }
    ...
    <div className="grid gap-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <Button size="lg" className="ml-2 gap-2" onClick={handleInstall} disabled={installLoading}>
          ...
          {installLoading ? m.skillActions.installing : m.skillActions.install.replace('{count}', String(downloadCount))}
        </Button>
      </div>
    </div>
  )
